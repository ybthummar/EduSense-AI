...
const VoiceRecorder = ({
  ...
  return (
    ...
    >
      ...
      <div className="w-full h-10 flex items-center justify-center gap-0.5 px-4 overflow-hidden">
...
export const PromptInputBox = React.forwardRef((props, ref) => {
  ...
  React.useEffect(() => {
    const currentRef = ref?.current || promptBoxRef.current;
    if (currentRef) {
        currentRef.addEventListener("paste", handlePaste);
        return () => currentRef.removeEventListener("paste", handlePaste);
    }
  }, [handlePaste, ref]);
