...
export default function Modal({ isOpen, onClose, title, children, maxWidth = 'max-w-lg' }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className={`relative w-full ${maxWidth} mx-4 glass-card p-6 animate-scale-in shadow-2xl shadow-black/30`}>
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-lg font-semibold text-surface-100">{title}</h3>
          <button
            onClick={onClose}
            className="text-surface-500 hover:text-surface-200 transition-colors cursor-pointer p-1.5 rounded-lg hover:bg-surface-800/50"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
